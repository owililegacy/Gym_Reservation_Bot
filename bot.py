...
"""
import os, sys, json, datetime as dt, logging, traceback
...
TIME_TO_RUN  = os.getenv("TIME_TO_RUN")  or "11:00"   # unused, just documentation
# -----------------------------------
...
def book_first_free_slot(driver):
    ...
    if not slots:
        ...
        return
    slots[0].click()
    WebDriverWait(driver, 15).until(EC.element_to_be_clickable((By.ID, "confirm-booking"))).click()
    WebDriverWait(driver, 15).until(EC.presence_of_element_located((By.CSS_SELECTOR, ".success")))
...
def main():
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler("./gym-bot.log"), logging.StreamHandler()],
    )
    driver = None
    try:
        driver = build_driver()
